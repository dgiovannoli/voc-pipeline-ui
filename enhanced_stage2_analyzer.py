...
For each quote, identify the TOP 1-3 most relevant executive criteria and provide:
- Primary criterion (the most relevant one)
- Secondary criterion (if applicable, 2nd most relevant)
- Tertiary criterion (if applicable, 3rd most relevant)
- Relevance scores (0-5 scale) for each identified criterion
- Sentiment per criterion (positive/negative/neutral/mixed) - each criterion can have different sentiment
- Overall sentiment (positive/negative/neutral/mixed) - weighted by relevance scores
- Priority level (critical/high/medium/low)
- Confidence level (high/medium/low)
- Brief explanation of why these criteria are most relevant

SENTIMENT ASSIGNMENT RULES:
- Each criterion should have its own sentiment based on what the customer said about that specific aspect
- Relevance score (0-5) indicates how much the customer talked about this criterion
- Sentiment (positive/negative/neutral/mixed) indicates the customer's attitude toward this criterion
...
- Overall sentiment should be weighted by relevance scores (higher relevance = more influence)
- If criteria have conflicting sentiments, overall sentiment should reflect the most relevant criterion
...
- Positive: Praise, satisfaction, success stories, compliments
- Negative: Complaints, dissatisfaction, problems, criticisms
- Neutral: Factual statements, neither positive nor negative
- Mixed: Contains both positive and negative elements

IMPORTANT: Be generous in identifying relevant criteria. If a customer mentions ANYTHING related to the criteria below, consider it relevant. Only mark as "not_relevant" if the quote is completely unrelated to business or product feedback.

The 10 executive criteria are:
1. product_capability: Functionality, features, performance, core solution fit, accuracy, ease of use
2. implementation_onboarding: Deployment ease, time-to-value, setup complexity, getting started
3. integration_technical_fit: APIs, data compatibility, technical architecture, connections
4. support_service_quality: Post-sale support, responsiveness, expertise, SLAs, customer service
5. security_compliance: Data protection, certifications, governance, risk management, privacy
6. market_position_reputation: Brand trust, references, analyst recognition, market standing
7. vendor_stability: Financial health, roadmap clarity, long-term viability, company stability
8. sales_experience_partnership: Buying process quality, relationship building, sales team
9. commercial_terms: Price, contract flexibility, ROI, total cost of ownership, pricing
10. speed_responsiveness: Implementation timeline, decision-making speed, agility, turnaround time

Respond with a JSON array where each element contains: quote_id, primary_criterion, secondary_criterion (null if none), tertiary_criterion (null if none), relevance_scores (object with only the relevant criteria), criterion_sentiments (object with sentiment per criterion), overall_sentiment, priority, confidence, explanation."""),
            ("user", batch_text)
        ])
    ...
    def _parse_llm_batch_response(self, llm_response, batch_df):
        ...
        try:
            ...
            if json_match:
                ...
                for idx, row in batch_df.iterrows():
                    quote_id = row['response_id'] if 'response_id' in row else f'quote_{idx}'
                    results.append(self._create_default_result(quote_id, "Default analysis - LLM response parsing failed"))
        except Exception as e:
            ...
            for idx, row in batch_df.iterrows():
                quote_id = row['response_id'] if 'response_id' in row else f'quote_{idx}'
                results.append(self._create_default_result(quote_id, f"Default analysis - Error: {str(e)}"))
        
        return results
    
    def _normalize_result_fields(self, result):
        """Ensure all required fields are present in the result"""
        # Ensure basic fields exist
        result.setdefault('primary_criterion', 'not_relevant')
        result.setdefault('secondary_criterion', None)
        result.setdefault('tertiary_criterion', None)
        result.setdefault('relevance_scores', {})
        result.setdefault('criterion_sentiments', {})
        result.setdefault('overall_sentiment', 'neutral')
        result.setdefault('priority', 'medium')
        result.setdefault('confidence', 'medium')
        result.setdefault('explanation', '')
        
        # If criterion_sentiments is missing, create it from relevance_scores
        if not result.get('criterion_sentiments') and result.get('relevance_scores'):
            criterion_sentiments = {}
            for criterion in result['relevance_scores'].keys():
                criterion_sentiments[criterion] = 'neutral'  # Default sentiment
            result['criterion_sentiments'] = criterion_sentiments
        
        # If overall_sentiment is missing, calculate it from criterion sentiments
        if not result.get('overall_sentiment') or result['overall_sentiment'] == 'neutral':
            result['overall_sentiment'] = self._calculate_overall_sentiment(
                ...
                result.get('criterion_sentiments', {})
            )
        
        return result
    
    def _calculate_overall_sentiment(self, relevance_scores, criterion_sentiments):
        """Calculate overall sentiment weighted by relevance scores"""
        if not relevance_scores or not criterion_sentiments:
            return 'neutral'
        ...
        total_weight = 0
        for criterion, relevance_score in relevance_scores.items():
...
For each quote, identify the TOP 1-3 most relevant executive criteria (not all 10) and provide:
- Primary criterion (the most relevant one)
- Secondary criterion (if applicable, 2nd most relevant)
- Tertiary criterion (if applicable, 3rd most relevant)
- Relevance scores (0-5 scale) for each identified criterion
...
Only include criteria that are actually mentioned or implied in the quote. If a quote doesn't clearly relate to any of the 10 criteria, mark it as "not_relevant".
...
Respond with a JSON array where each element contains: quote_id, primary_criterion, secondary_criterion (null if none), tertiary_criterion (null if none), relevance_scores (object with only the relevant criteria), criterion_sentiments (object with sentiment per criterion), overall_sentiment, priority, confidence, explanation."""),
    ...
    def _calculate_overall_sentiment(self, relevance_scores, criterion_sentiments):
        ...
        # If there's a clear winner (more than 50% of total weight)
        if max_sentiment[1] > total_weight * 0.5:
            return max_sentiment[0]
        else:
            # If no clear winner, check for mixed
    ...
    def _create_default_result(self, quote_id, explanation):
        """Create a default result for fallback cases"""
        return {
            'quote_id': quote_id,
    ...
    def _save_batch_results_to_database(self, results, client_id):
        """Save batch results to database - 1 record per quote"""
        if not self.supabase:
            return
        print(f"[DEBUG] Saving batch results: type(results)={type(results)}, len={len(results)}")
        for idx, result in enumerate(results):
            print(f"[DEBUG] Result {idx}: type={type(result)}, value={result}")
            if not isinstance(result, dict):
                print(f"[ERROR] Skipping non-dict result: {result}")
                continue
            
            # Get primary criterion and relevance scores
            primary_criterion = result.get('primary_criterion', 'not_relevant')
            relevance_scores = result.get('relevance_scores', {})
            criterion_sentiments = result.get('criterion_sentiments', {})
            overall_sentiment = result.get('overall_sentiment', 'neutral')
            
            if primary_criterion == 'not_relevant' or not relevance_scores:
                print(f"[INFO] Skipping quote {result.get('quote_id', 'unknown')} - not relevant to any criteria")
                continue
            
            # Get the score and sentiment for the primary criterion
            ...
            # Create enhanced explanation with all criteria info
            ...
            # Create single record per quote
            db_record = {
                'quote_id': result.get('quote_id', ''),
                'criterion': primary_criterion,
                'relevance_score': int(primary_score),  # Convert to integer for database constraint
                'sentiment': primary_sentiment,  # Use primary criterion sentiment
                'priority': result.get('priority', 'medium'),
                'confidence': result.get('confidence', 'medium'),
                'relevance_explanation': json.dumps(enhanced_explanation),  # Store all info as JSON
