...
class SupabaseDatabase:
    ...
    def _fetch_llm_theme_similarity(self, client_id: str, min_score: float = 0.7) -> pd.DataFrame:
        ...
        try:
            ...
            if not api_key:
                self.logger.warning("⚠️ OpenAI API key not found, falling back to rule-based similarity")
            ...
            try:
                ...
                self.logger.warning(f"⚠️ Could not fetch rollup interview themes: {e}")
...
Return only valid JSON."""
            ...
            if json_start == -1 or json_end == -1:
                self.logger.warning("⚠️ Could not extract JSON from LLM response")
                return self._fetch_rule_based_similarity(client_id, min_score)
            ...
            self.logger.warning(f"⚠️ LLM similarity analysis failed: {e}, falling back to rule-based")
            return self._fetch_rule_based_similarity(client_id, min_score)
