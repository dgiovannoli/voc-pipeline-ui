#!/usr/bin/env python3
"""
Stage 4 Theme Analyzer V2 - Improved Version
Enhanced V2 approach with higher token limits and better cross-company validation
"""
...
class Stage4ThemeAnalyzerV2Improved:
    ...
    def _call_stage4a_pattern_identification(self, findings_json: str) -> str:
        """Stage 4A: Identify broad patterns with enhanced requirements"""
        try:
            pattern_prompt = f"""
You are an expert qualitative research analyst specializing in B2B SaaS competitive intelligence.

TASK: Identify 8-15 broad patterns from the provided findings data with STRONG cross-company validation.
...
- Generate comprehensive coverage for detailed theme development
...
- Generate 8-15 patterns for comprehensive coverage
- Ensure patterns cover diverse business areas (pricing, efficiency, security, etc.)
...
- Generate 3-4 detailed themes for each pattern (aim for 15-25 total themes)
...
- Generate 5-8 strategic alerts for comprehensive coverage
...
- Use the exact finding IDs from the pattern's supporting_finding_ids for themes
...
"""
    ...
    def analyze_themes_v2_improved(self) -> bool:
        """Main method to analyze findings using improved V2 approach"""
        try:
            logger.info(f"🎯 Starting Stage 4 V2 Improved theme analysis for client: {self.client_id}")
            ...
            # Stage 4A: Enhanced Pattern Identification
            logger.info("🔄 Stage 4A: Identifying patterns with enhanced requirements...")
            ...
            if success:
                logger.info(f"✅ Stage 4 V2 Improved theme analysis completed successfully for client {self.client_id}")
                logger.info(f"📊 Generated {len(themes)} themes/alerts using improved V2 approach")
            else:
            ...
            logger.error(f"Error in improved V2 theme analysis: {e}")
            return False
    
    def process_themes_v2_improved(self, client_id: str = None) -> bool:
        """Process themes using improved V2 approach for a specific client"""
        if client_id:
            self.client_id = client_id
        
        return self.analyze_themes_v2_improved()

def main():
    """Main function to run Stage 4 V2 Improved theme analysis"""
    import sys
    import argparse
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Stage 4 V2 Improved Theme Analysis')
    ...
    try:
        analyzer = Stage4ThemeAnalyzerV2Improved(client_id)
        success = analyzer.process_themes_v2_improved()
        
        if success:
            print(f"✅ Stage 4 V2 Improved theme analysis completed successfully for client: {client_id}")
            sys.exit(0)
        else:
            print(f"❌ Stage 4 V2 Improved theme analysis failed for client: {client_id}")
            sys.exit(1)
            
    except Exception as e:
        print(f"❌ Error running Stage 4 V2 Improved theme analysis: {e}")
        sys.exit(1)
