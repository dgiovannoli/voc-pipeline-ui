#!/usr/bin/env python3
"""
Stage 4 Theme Analyzer v2 - Two-Stage Processing Approach
Generates themes from Stage 3 findings using enhanced two-stage processing
Stage 4A: Pattern Identification
Stage 4B: Granular Theme Development
"""
...
class Stage4ThemeAnalyzerV2:
    ...
    def _call_stage4a_pattern_identification(self, findings_json: str) -> str:
        """Stage 4A: Identify broad patterns from findings"""
        try:
            pattern_prompt = f"""
You are an expert qualitative research analyst specializing in B2B SaaS competitive intelligence.

TASK: Identify 8-12 broad patterns from the provided findings data.
...
- Each pattern should be supported by findings from 3+ companies
...
"""
            ...
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are an expert business analyst specializing in pattern identification from customer research data."},
    ...
    def _call_stage4b_theme_development(self, patterns_json: str, findings_json: str) -> str:
        """Stage 4B: Develop granular themes from patterns"""
        try:
            theme_prompt = f"""
You are an expert qualitative research analyst specializing in B2B SaaS competitive intelligence.

TASK: Develop granular themes and strategic alerts from the identified patterns.
...
- Generate 2-3 detailed themes for each pattern
...
"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert business analyst specializing in theme development and strategic analysis."},
    ...
    def analyze_themes_two_stage(self) -> bool:
        """Main method to analyze findings using two-stage processing"""
        try:
            logger.info(f"🎯 Starting Stage 4 v2 (Two-Stage) theme analysis for client: {self.client_id}")
            ...
            # Stage 4A: Pattern Identification
            logger.info("🔄 Stage 4A: Identifying patterns...")
            ...
            # Stage 4B: Theme Development
            logger.info("🔄 Stage 4B: Developing themes...")
            ...
            if success:
                logger.info(f"✅ Stage 4 v2 (Two-Stage) theme analysis completed successfully for client {self.client_id}")
                logger.info(f"📊 Generated {len(themes)} themes/alerts using two-stage processing")
            else:
            ...
            logger.error(f"Error in two-stage theme analysis: {e}")
            return False
    
    def process_themes_v2(self, client_id: str = None) -> bool:
        """Process themes using two-stage approach for a specific client"""
        if client_id:
            self.client_id = client_id
        
        return self.analyze_themes_two_stage()

def main():
    """Main function to run Stage 4 v2 theme analysis"""
    import sys
    import argparse
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Stage 4 v2 Theme Analysis (Two-Stage)')
    ...
    try:
        analyzer = Stage4ThemeAnalyzerV2(client_id)
        success = analyzer.process_themes_v2()
        
        if success:
            print(f"✅ Stage 4 v2 (Two-Stage) theme analysis completed successfully for client: {client_id}")
            sys.exit(0)
        else:
            print(f"❌ Stage 4 v2 (Two-Stage) theme analysis failed for client: {client_id}")
            sys.exit(1)
            
    except Exception as e:
        print(f"❌ Error running Stage 4 v2 (Two-Stage) theme analysis: {e}")
        sys.exit(1)
