...
Generates high-quality themes from Stage 3 findings using improved validation and quality controls
BEST PRACTICE: LLM generates themes/alerts, code validates cross-company patterns and scores evidence strength
"""
...
class EnhancedStage4ThemeAnalyzer:
    def __init__(self, client_id: str = "Rev"):
        ...
        # Load enhanced theme prompt
        self.theme_prompt = self._load_enhanced_theme_prompt()
    ...
    def _load_enhanced_theme_prompt(self) -> str:
        """Load the enhanced theme prompt"""
        return """
B2B SaaS Win/Loss Theme Development Protocol - ENHANCED

CRITICAL REQUIREMENTS:
1. ONLY create themes when you find REAL cross-company patterns (2+ companies minimum)
2. Each theme MUST be based on actual evidence from the findings
3. Do NOT create artificial patterns or force cross-company validation
4. If no real cross-company patterns exist, generate fewer themes but higher quality

QUALITY STANDARDS:
- Theme statements should be 75-150 words (not artificially constrained)
- Include specific company names and examples from findings
- Use real quotes and evidence, not generic language
- Focus on actionable business insights

VALIDATION PROCESS:
1. Identify actual patterns across 2+ companies
2. Extract specific examples and quotes
3. Generate natural narrative (not template-driven)
4. Verify evidence strength before including
    ...
    "themes": [
        {
            ...
            "theme_statement": "Natural narrative with real evidence (75-150 words)",
            "classification": "REVENUE_THREAT|COMPETITIVE_VULNERABILITY|MARKET_OPPORTUNITY",
            "supporting_finding_ids": "F1,F2,F3",
            "primary_quote": "Real quote from findings",
            "secondary_quote": "Supporting quote from findings",
    ...
    "strategic_alerts": [
        {
            ...
            "alert_statement": "Detailed narrative about specific company insight",
            "alert_classification": "REVENUE_THREAT|COMPETITIVE_VULNERABILITY|MARKET_OPPORTUNITY",
            "supporting_alert_finding_ids": "F1",
            "primary_alert_quote": "Real quote from finding"
        }
    ]
}

CRITICAL: Only generate themes for real cross-company patterns. If no real patterns exist, generate fewer themes but ensure each one is high-quality and actionable.
"""
    ...
    def _call_llm_for_themes(self, findings_json: str) -> str:
        """Call OpenAI API to generate themes using the enhanced protocol"""
        try:
...
4. Generate HIGH-QUALITY themes with specific details and actionable insights
...
QUALITY THEME REQUIREMENTS:
- Generate 5-10 high-quality themes instead of 15-25 generic themes
...
- Use natural narrative (75-150 words) instead of rigid templates
...
- Generate 5-10 high-quality themes to capture detailed patterns.
...
- Theme statements MUST be 75-150 words (natural length, not artificially constrained)
- Every theme statement MUST include cross-company validation (e.g., "across 3 companies", "spanning 4 organizations")
...
"""
            ...
            logger.info(f"✅ LLM generated theme response ({len(llm_output)} characters)")
            
            return llm_output
            
        except Exception as e:
            logger.error(f"❌ Error calling LLM for themes: {str(e)}")
            return ""
    ...
    def analyze_themes(self) -> bool:
        """Main method to analyze findings and generate themes with enhanced quality controls"""
        try:
            logger.info(f"🎯 Starting Enhanced Stage 4 theme analysis for client: {self.client_id}")
            ...
            if success:
                logger.info(f"✅ Enhanced Stage 4 theme analysis completed successfully for client {self.client_id}")
            else:
            ...
            logger.error(f"Error in enhanced theme analysis: {e}")
            return False
    
    def _generate_themes_from_findings(self, findings: List[Dict]) -> List[Dict]:
        """Generate themes from findings using LLM with enhanced protocol"""
        try:
            if not findings:
...
def main():
    """Main function to run enhanced Stage 4 theme analysis"""
    import sys
    
    client_id = sys.argv[1] if len(sys.argv) > 1 else "Rev"
    
    analyzer = EnhancedStage4ThemeAnalyzer(client_id=client_id)
    success = analyzer.process_themes()
    
    if success:
        print(f"✅ Enhanced Stage 4 theme analysis completed successfully for client: {client_id}")
    else:
        print(f"❌ Enhanced Stage 4 theme analysis failed for client: {client_id}")
