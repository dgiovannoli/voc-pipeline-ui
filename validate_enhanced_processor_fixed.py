...
def test_full_processor():
    ...
    try:
        ...
        try:
            # Run the processor as a subprocess to capture output
            result = subprocess.run([
                'python', 'voc_pipeline/processor.py',
                temp_path, 'test_client', 'Test Company', 'Test Person', 'Won', '2024-01-15'
            ], capture_output=True, text=True, cwd='.')
            
            if result.returncode == 0 and result.stdout:
                # Check if the output contains timestamp columns
                if "start_timestamp" in result.stdout and "end_timestamp" in result.stdout:
                    print("✅ Full processor test passed!")
                    print("✅ Result contains timestamp columns")
                    return True
                else:
                ...
                print(f"❌ Full processor test failed - return code: {result.returncode}")
